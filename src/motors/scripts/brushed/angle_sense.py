...
class AngleSensor:
	...
	def _read(self, mux, data_rate, mode):
		...
		self.bus.write_i2c_block_data(ADS1x15_DEFAULT_ADDRESS, ADS1x15_POINTER_CONFIG, [(config >> 8) & 0xFF, config & 0xFF])
		# Wait for the ADC sample to finish based on the sample rate plus a
		# small offset to be sure (0.1 millisecond).
		time.sleep(1.0/data_rate+0.0001)
		# Retrieve the result.
		result = self.bus.read_i2c_block_data(ADS1x15_DEFAULT_ADDRESS, ADS1x15_POINTER_CONVERSION, 2)
